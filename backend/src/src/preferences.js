...
class Preferences {
  ...
  directory() {
    ...
    const platform =
      (typeof process !== 'undefined' && process.platform) || import.meta.env.PLATFORM;

    if (platform === 'darwin') {
      const home =
        app && typeof app.getPath === 'function'
          ? app.getPath('home')
          : process.env.HOME || import.meta.env.HOME || process.cwd();
    ...
    const appData =
      app && typeof app.getPath === 'function'
        ? app.getPath('appData')
        : process.env.APPDATA || import.meta.env.APPDATA || process.cwd();
