...
import { arrayToBuilds, shorthandSkills } from '../../../src/common/formatters.js';
class WebLogger {
  error(...a) {
...
export async function getVersion() {
  const i = await fetch('https://api.koreanbuilds.net/champions?patchid=-1', {
    headers: headers,
  }).then(res => res.json());
    ...
    .then(champions => {
      return champions
        ...
        .map(
          champ_data => {
            ...
            if (Array.isArray(numberOfGamesPerRole)) {
              ...
              roles = Object.keys(numberOfGamesPerRole).filter(k => {
              ...
              roles = usage
                .map(u => (typeof u === 'string' ? u : u.role || u.name))
                .filter(Boolean);
        ...
        .map(
          champ_data => {
            logger.info(
              `${T.t('processing')} Koreanbuilds: ${T.t(
                champ_data.formatted_name.toLowerCase().replace(/[^a-z]/g, '')
              )}`
            );
            ...
            return Promise.resolve(champ_data.roles)
              .map(role =>
                fetch(
                  `https://api.koreanbuilds.net/builds?chmpname=${champ_data.name}&patchid=-1`,
                  { headers: headers }
                )
                  .then(c => c.json())
                  .then(champ_build => {
                    ...
                    confident_builds.forEach(build => {
                      ...
                      const skillOrderRaw =
                        build.skillSets &&
                        (Array.isArray(build.skillSets.skillOrder)
                          ? build.skillSets.skillOrder[0]
                          : build.skillSets.skillOrder);
                      const _skillMap = { 1: 'Q', 2: 'W', 3: 'E', 4: 'R' };
                      let parsedSkillOrder = '';

                      if (typeof skillOrderRaw === 'string') {
                        const mapped = skillOrderRaw
                          .split('')
                          .map(d => _skillMap[d])
                          .filter(Boolean);
                      ...
                      const block = [
                        {
                          ...
                          type: `${T.t('starter', true)} - ${stats} - ${games_played} ${T.t(
                            'games_played',
                            true
                          )}`,
                        },
                      ...
                      const riot_json = R.mergeRight(R.clone(default_schema, true), {
                        ...
                        blocks: trinksCon(block, {
                          highest_win: parsedSkillOrder,
                          most_freq: parsedSkillOrder,
                        }),
                      });
