...
class Translate {
  ...
  async loadPhrases(locale) {
    ...
    const translationsBaseFs = path.join(process.cwd(), 'public', 'translations');
    ...
    try {
      ...
      const translationsBase = new URL('../../../public/translations', import.meta.url);
...
export async function lokaliseUpload() {
  ...
  const translationsPath = path.join(__dirname, '../public/translations');
...
export async function lokaliseReview() {
  ...
  New Trans   | ${chalk.bold.magenta(newTrans)}`);
    ...
    const { answer } = await prompt.get([
      {
        name: 'answer',
        description: 'Would you like to save these translations? [y/n]',
        pattern: /^[yn]$/,
        required: true,
      },
    ]);
