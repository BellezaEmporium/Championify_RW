...
import { arrayToBuilds, pickWinrate, pickPickrate } from '../../../common/formatters.js';
...
export async function getVersion() {
  const data = await fetch('https://lol-api-champion.op.gg/api/meta/champions', { headers: headers })
  .then(resp => resp.json())
...
export function getSr() {
  ...
  return fetch('https://lol-api-champion.op.gg/api/euw/champions/ranked', { headers: headers })
    ...
    .then(data => {
      ...
      return Promise.map(champs, champ_data => {
        cl(`${T.t('processing')} op.gg: ${T.t(champ_data.name)}`);
        return Promise.map(champ_data.positions, position => {
          return Promise.join(
            fetch(`https://lol-api-champion.op.gg/api/euw/champions/ranked/${champ_data.id}/${position}/item`, { headers: headers }),
            fetch(`https://lol-api-champion.op.gg/api/euw/champions/ranked/${champ_data.id}/${position}/skill`, { headers: headers })
          )
            .spread((i, s) => {
              const skills = s.pageProps.data.skills;
              const starter = s.pageProps.data.starter_items;
              const core = s.pageProps.data.core_items;
              const items = s.pageProps.data.items;
              let boots = s.pageProps.data.boots;
