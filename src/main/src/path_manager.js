...
function checkInstallPath(selected_path, done) {
  ...
  if (process.platform === 'darwin') {
    ...
    if (fs.existsSync(newLauncher)) {
      done(null, selected_path, 'Config/Champions/', path.basename(newLauncher));
    } else if (fs.existsSync(oldLauncher)) {
      done(null, selected_path, 'Config/Champions/', path.basename(oldLauncher));
    } else if (fs.existsSync(garenaExe)) {
      done(null, selected_path, 'Game/Config/Champions/', path.basename(garenaExe));
    } else if (garenaLauncher) {
      const garenaApp = glob.sync(path.join(selected_path, 'GameData/Apps/*'))[0];
      let garenaVersion = garenaApp ? path.basename(garenaApp) : '';
      done(
        null,
        selected_path,
